...
def process(api_key, region, name=-1):
    global games
    while (games):
        game = games.pop()
        size = len(games)
        ...
        if not path.isfile(filename):
            print "[Thread %d] Downloading %s (%d left)" % (name, filename, size)
            ok = False
            while not ok:
                ...
                if r.status_code == 429:
                    ...
                    print 'received bad status code %s for game %s, retrying in a minute' % (game, r.status_code)
                    sleep(60)
...
for f in [f for f in os.listdir(input_directory) if path.isfile(path.join(input_directory, f))]:
    ...
    for apikey in api_keys:
        threads.append(threading.Thread(target=process, args=[apikey, region, q]))
        q += 1
